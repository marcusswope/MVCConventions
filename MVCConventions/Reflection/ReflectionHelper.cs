...
namespace MVCConventions.Reflection
{
    public static class ReflectionHelper
    {
        ...
        public static string PropertyName<TModel>(this Expression<Func<TModel, object>> expression)
        {
            MemberExpression memberExpression = getMemberExpression(expression);
        ...
        {
            MemberExpression memberExpression = GetMemberExpression(expression, true); return GetAccessor(memberExpression);
        }
        ...
        public static MemberExpression GetMemberExpression(this LambdaExpression expression, bool enforceMemberExpression)
        {
            ...
            if (expression.Body.NodeType == ExpressionType.Convert)
            {
                var body = (UnaryExpression)expression.Body; memberExpression = body.Operand as MemberExpression;
            }
            else if (expression.Body.NodeType == ExpressionType.MemberAccess)
            {
                memberExpression = expression.Body as MemberExpression;
        ...
        private static void buildValueGetters(Expression expression, IList<IValueGetter> list)
        {
            ...
            if (methodCallExpression != null)
            {
                ...
                var firstArgumentConstantExpression = firstArgument as ConstantExpression; 
                if (firstArgumentConstantExpression != null)
                {
                    var value = firstArgumentConstantExpression.Value; 
                    var methodValueGetter = new MethodValueGetter(methodInfo, value); 
                    list.Add(methodValueGetter);
                }
                
                var constantExpressionProperty = firstArgument.GetType().GetProperties().FirstOrDefault(x => x.Name == "Expression" && x.PropertyType == typeof(Expression));
                var memberInfoProperty = firstArgument.GetType().GetProperties().FirstOrDefault(x => x.Name == "Member" && x.PropertyType == typeof(MemberInfo));
                if (constantExpressionProperty != null && memberInfoProperty != null)
                {
                    var constantExpression = constantExpressionProperty.GetValue(firstArgument, null) as ConstantExpression;
                    var memberInfo = memberInfoProperty.GetValue(firstArgument, null) as MemberInfo;
                    if (constantExpression != null && memberInfo != null)
                    {
                        var page = constantExpression.Value;
                        var fieldInfo = page.GetType().GetFields().First(x => x.Name == memberInfo.Name);
                        var methodValueGetter = new MethodValueGetter(methodInfo, fieldInfo.GetValue(page));
                        list.Add(methodValueGetter);
                    }
                }
    ...
    public abstract class ExpressionVisitorBase
    {
        ...
        {
            Expression exp = Visit(m.Expression); if (exp != m.Expression) { return Expression.MakeMemberAccess(exp, m.Member); } return m;
        }
